...
> 規格表原列 Spring Boot 4.1.0 / Java 21；本專案即以此為準（4.1.0 GA 於 2026-06，Java 17+ baseline）。
...
（無 Docker 也可：自備 PostgreSQL 16 並安裝 pgvector 擴充；Redis 僅 `redis` profile 需要。）
...
```bash
# Windows PowerShell
$env:JAVA_HOME='C:\Users\<you>\.jdks\openjdk-21.x'
mvn spring-boot:run
